...
// 根据像素获取对应帧数
export const getFrameCount = (scale: number, pixel: number) => {
    const gridPixel = getGridSize(scale);
    let gridCount = pixel / gridPixel;
    if (scale < 70) { // 1秒一格
        gridCount = gridCount * 30;
    }
    if (scale < 30) { // 6秒一格
        gridCount = gridCount * 6;
    }
    return Math.round(gridCount);
};
// 根据缩放比调整 step
