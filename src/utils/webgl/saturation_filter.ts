...
const fragment = `
  ...
  uniform float uSaturation;   // 饱和度系数（由 CPU 传递，默认 1.0）
  ...
  void main() {
    ...
    vec3 grayColor = vec3(gray); // 灰色的 RGB 分量
    
    // 3. 混合灰色与原始颜色，调整饱和度
    color.rgb = mix(grayColor, color.rgb, uSaturation);
...
const SaturationFilter = new Filter({
    // Shader programs
